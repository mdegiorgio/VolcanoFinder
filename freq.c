...
void makeguess_unnormalized(double *p, char *fn) {
  int i, mini=1;
  double sum=0.0;
  struct rdatatype *curr;
...
void getfreq_unnormalized(char *outfn) {
  ...
  get_rdata(NULL);
  printf("getfreq_unnormalized rdatasize=%i\n", rdatasize);
  if (rdatasize<=0) {
    fprintf(stderr, "getfreq_unnormalized_nompi rdataisze=%i\n", rdatasize);
    exit(-1);
  }
  p=malloc(xmax*sizeof(double));
  makeguess_unnormalized(p, NULL);
  if (invar==2) numpar=xmax-1;
  else numpar = xmax-2;
  lowbound=malloc(numpar*sizeof(double));
  upbound=malloc(numpar*sizeof(double));
  ...
  //  findmax(invec, numpar, p_likelihood_freq, d_likelihood_freq,
  //	  lowbound, upbound, 1);
  //get_p(p, invec); // COMMENTED THIS OUT
  outfile = my_fopen(outfn, "w");
  for (i=0; i<xmax; i++)
    fprintf(outfile, "%i\t%e\n", i, p[i]);
  fclose(outfile);
